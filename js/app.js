...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    // Create element to display produt and it's data

    const div = document.createElement("div");
    div.classList.add("col-md-4");
    div.innerHTML = `<div class="rounded mt-12 overflow-hidden border p-2" >
    <img src=${image} style= "height:300px;width:100px;"  
    class="w-100"
    alt="">
    </img>
      </div>
      ...
      <h2 class="text-purple-700">Price: $ ${product.price}</h2>
      <h5>Total-Rating: <span class="text-green-700">${count}   </span></h5>
      <h5>Rating: ${rate}</h5>
      ...
      rounded
      mr-1
      mb-1">add to cart</button>
      <button id="details-btn" onclick='showDetails(${product.price},${rate})' 
      class=" bg-purple-700
      text-white
      text-lg
      font-bold
